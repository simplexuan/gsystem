 ...
 * @license
 */
class Model_gender extends Gsystem_model {
				/**
	 * _model 
	 * 
	 * @var string
	...
	protected $_mkeys = array(
			'Gsystem_Model_gender_name:%s' => array('name', ),
			'Gsystem_Model_gender_short:%s' => array('short', ),
			 
			'Gsystem_Model_gender' => array(),
			);
	/**
	 * _equal_search_items 
	 * 
	 ...
	 * @access protected
	 */
	protected $_equal_search_items = array('name'=>'t','short'=>'t',);
		/**
	 * __construct
	 *
	 * @access protected
	...
	function new_c($id = 0) {
		...
		$new_gender  = array();
		if ($id > 0) {
			$new_gender['gender'] = $this->dao('/Dao_gender', $dao_param)->fetch_one_by_id($id);
		} else {
			$new_gender['gender']  = $this->dao('/Dao_gender', $dao_param)->new_one();
		}
								return $new_gender;
	}
						/**
	...
	function save($param = array()) {
		...
		if (!isset($param[$this->_model])) {
			...
			if ($gender_id > 0) {
				unset($gender['id']);
		        $old_gender = $this->dao('/Dao_gender', $dao_param)->fetch_one_by_id($gender_id);
				//$this->_history($gender_id, $gender);
				$this->dao('/Dao_gender', $dao_param)->update($gender, $gender_id);
				$is_update = TRUE;
			} else {
				$old_gender  = array();
				$gender_id = $this->dao('/Dao_gender', $dao_param)->insert($gender);
	            			}

									
			
			//清除缓存
		foreach (array($old_gender, array_merge($old_gender, $gender)) as $v){
	...
	function search($param = array(), $page = 0, $pagesize = 0) {
		$dao_param = array('active_group'=>$this->config->item('active_group'), 'id'=>0);
		$genders = array('num'=>0, 'results'=>array());
		...
		if (isset($param['_ft_']) && !empty($param['_ft_'])){
			$genders  = $this->dao('public/Dao_searcher', 
			...
			if ($key === FALSE || ($page * $pagesize) > self :: ID_CACHE_NUM ){
				//    $param['ordersort']     =  'created_at DESC';
				$genders = $this->dao('/Dao_gender', $dao_param )->search($param);
			}else{ //缓存
				$genders = $this->cache->memcached->get($key);
				if (empty($genders)) {
					$param['page']     = 1;
					$param['pagesize'] = self :: ID_CACHE_NUM;
					$genders = $this->dao('/Dao_gender', $dao_param )->search($param);
					if (!$this->cache->memcached->save($key, $genders)){
				...
				if ($page > 0 && $pagesize > 0) {
					// array_slice 获取需要的数据
					$limit = $pagesize;
					if ($genders['num'] < $page* $pagesize){
						$limit = $genders['num'] - ($page-1) * $pagesize;
					}
					$genders['results'] = array_slice($genders['results'], ($page-1) * $pagesize, $limit, TRUE);
				}
			}
			//$genders = $this->dao('/Dao_gender', $dao_param )->search($param);
		}
		if ( $genders['num'] > 0) {
			$items = $this->dao('/Dao_gender', $dao_param)
				->get_multi(array_keys($genders['results']), $selected);
			foreach ($items as $item) {
				$genders['results'][$item['id']] = $item;
			}
		}
		return  $genders;
	}
	...
	function delete_one_by_id($id=0, $user_id=0) {
		...
		$gender = $this->dao('/Dao_gender', $dao_param)->fetch_one_by_id($id);
		if ($user_id >0 && $gender['user_id'] != $user_id){
			throw new Exception(sprintf('function: %s, op:%d has no permission to delete user_id:%d', __FUNCTION__,
				...
				$this->dao('/Dao_gender', $dao_param)->delete_one_by_id($id);
			foreach($this->_mkeys as $k=>$items){
				$temp = array();
				foreach($items as $item){
					$temp[$item] = $gender[$item];
				}
				$key = vsprintf($k, $temp);
				if (!$this->cache->memcached->del($key)){
	...
	function update_by_id($param = array(), $id = 0) {
		...
		$gender = $this->dao('/Dao_gender', $dao_param)->fetch_one_by_id($id);
				$this->dao('/Dao_gender', $dao_param)->update($param, $id);
		//清除缓存
		foreach (array($gender, array_merge($gender, $param)) as $v){
			foreach ($this->_mkeys as $key_pattern => $keys){
				$temp = array();
				foreach($keys as $key){
					$temp[$key] = $v[$key];
	...
	function __call($func, $args) {
		$dao_param = array('active_group'=>$this->config->item('active_group'), 'id'=>0);
		return call_user_func_array(array($this->dao('/Dao_gender',  $dao_param), $func), $args);
	}
	...
	function update_by_unique($param) {
		$dao_param = array('active_group'=>$this->config->item('active_group'), 'id'=>0);
		unset($param['id']);
				$id  = $this->dao(sprintf('/Dao_%s', 'gender'), $dao_param)->update_by_unique($param);
				if (intval($id)<=0){
						$param = array_merge($this->dao(sprintf('/Dao_%s', 'gender'), $dao_param)->new_one(), $param);
			$this->dao(sprintf('/Dao_%s', 'gender'), $dao_param)->insert($param);
		}else{
			$param['id'] = $id;
