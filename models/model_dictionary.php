 ...
 * @license
 */
class Model_dictionary extends Gsystem_model {
    /**
     * _model 
     * 
     * @var string
    ...
    protected $_mkeys = array(
        'Gsystem_Model_dictionary_type:%s$sort:%d' => array('type', 'sort', ),
        'Gsystem_Model_dictionary_type:%s' => array('type', ),
        'Gsystem_Model_dictionary' => array(),
    );
    /**
     * _equal_search_items 
     * 
     ...
     * @access protected
     */
    protected $_equal_search_items = array('type'=>'t','sort'=>'t',);
    /**
     * __construct
     *
     * @access protected
    ...
    function new_c($id = 0) {
        ...
        if ($id > 0) {
            $new_dictionary['dictionary'] = $this->dao('/Dao_dictionary', $dao_param)->fetch_one_by_id($id);
        } else {
            $new_dictionary['dictionary']  = $this->dao('/Dao_dictionary', $dao_param)->new_one();
        }
    ...
    function save($param = array()) {
        ...
        $dictionary_id = isset($dictionary['id']) ? $dictionary['id'] : 0;
        $is_update = FALSE;
        if ($dictionary_id > 0) {
            unset($dictionary['id']);
            $old_dictionary = $this->dao('/Dao_dictionary', $dao_param)->fetch_one_by_id($dictionary_id);
            $this->dao('/Dao_dictionary', $dao_param)->update($dictionary, $dictionary_id);
            $is_update = TRUE;
        } else {
            $old_dictionary  = array();
            $dictionary_id = $this->dao('/Dao_dictionary', $dao_param)->insert($dictionary);
        }



        //清除缓存
        foreach (array($old_dictionary, array_merge($old_dictionary, $dictionary)) as $v){
    ...
    function search($param = array(), $page = 0, $pagesize = 0) {
        $dao_param = array('active_group'=>$this->config->item('active_group'), 'id'=>0);
        $dictionarys = array('num'=>0, 'results'=>array());
        ...
        if (isset($param['_ft_']) && !empty($param['_ft_'])){
            $dictionarys  = $this->dao('public/Dao_searcher', 
            ...
            if ($key === FALSE || ($page * $pagesize) > self :: ID_CACHE_NUM ){
                ...
                $dictionarys = $this->dao('/Dao_dictionary', $dao_param )->search($param);
            }else{ //缓存
                $dictionarys = $this->cache->memcached->get($key);
                if (empty($dictionarys)) {
                    $param['page']     = 1;
                    $param['pagesize'] = self :: ID_CACHE_NUM;
                    $dictionarys = $this->dao('/Dao_dictionary', $dao_param )->search($param);
                    if (!$this->cache->memcached->save($key, $dictionarys)){
                ...
                if ($page > 0 && $pagesize > 0) {
                    // array_slice 获取需要的数据
                    $limit = $pagesize;
                    if ($dictionarys['num'] < $page* $pagesize){
                        $limit = $dictionarys['num'] - ($page-1) * $pagesize;
                    }
                    $dictionarys['results'] = array_slice($dictionarys['results'], ($page-1) * $pagesize, $limit, TRUE);
                }
            }
            //$dictionarys = $this->dao('/Dao_dictionary', $dao_param )->search($param);
        }
        if ( $dictionarys['num'] > 0) {
            $items = $this->dao('/Dao_dictionary', $dao_param)
                ->get_multi(array_keys($dictionarys['results']), $selected);
            foreach ($items as $item) {
                $dictionarys['results'][$item['id']] = $item;
            }
        }
        return  $dictionarys;
    }
    ...
    function delete_one_by_id($id=0, $user_id=0) {
        ...
        $dictionary = $this->dao('/Dao_dictionary', $dao_param)->fetch_one_by_id($id);
        if ($user_id >0 && $dictionary['user_id'] != $user_id){
        ...
        $this->dao('/Dao_dictionary', $dao_param)->delete_one_by_id($id);
    ...
    function update_by_id($param = array(), $id = 0) {
        ...
        $dictionary = $this->dao('/Dao_dictionary', $dao_param)->fetch_one_by_id($id);
        $this->dao('/Dao_dictionary', $dao_param)->update($param, $id);
        //清除缓存
        foreach (array($dictionary, array_merge($dictionary, $param)) as $v){
            foreach ($this->_mkeys as $key_pattern => $keys){
                $temp = array();
                foreach($keys as $key){
                    $temp[$key] = $v[$key];
    ...
    function __call($func, $args) {
        $dao_param = array('active_group'=>$this->config->item('active_group'), 'id'=>0);
        return call_user_func_array(array($this->dao('/Dao_dictionary',  $dao_param), $func), $args);
    }
    ...
    function update_by_unique($param) {
        $dao_param = array('active_group'=>$this->config->item('active_group'), 'id'=>0);
        unset($param['id']);
        $id  = $this->dao(sprintf('/Dao_%s', 'dictionary'), $dao_param)->update_by_unique($param);
        if (intval($id)<=0){
            $param = array_merge($this->dao(sprintf('/Dao_%s', 'dictionary'), $dao_param)->new_one(), $param);
            $this->dao(sprintf('/Dao_%s', 'dictionary'), $dao_param)->insert($param);
        }else{
            $param['id'] = $id;
