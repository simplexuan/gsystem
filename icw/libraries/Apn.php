...
/*******************************
	...
	protected $apnResonses = array(
		...
		300001 => 'Processing error',
		300002 => 'Missing device token',
		300003 => 'Missing topic',
		300004 => 'Missing payload',
		300005 => 'Invalid token size',
		300006 => 'Invalid topic size',
		300007 => 'Invalid payload size',
		300008 => 'Invalid token',
		30000255 => 'None (unknown)',
	);
	...
	*/
	protected function connect($server) {
		...
		if (!$stream) {
			...
			stream_set_timeout($stream, 3600);
	...
	function getPayloadStatuses()
	{
		...
		elseif ($changedStreams > 0)
		{
			...
			if ($responseBinary !== false || strlen($responseBinary) == 6) {
				...
				{
					log_message('error','APN: debugPayload response - status_code:'.$response['status_code'].' => '.$this->apnResonses[$response['status_code']+300000]);
					$this->error = $this->apnResonses[$response['status_code']+300000];
                    throw new Exception($this->error,  $response['status_code']+300000);
					//return false;
				}
				else
				{
