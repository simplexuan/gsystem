<?php 
class Logic_degree extends Gsystem_logic {
	/**
	 * __construct 
	 * 
	 * @access protected
	...
	function __call($func, $args) {
		return call_user_func_array(array($this->model('Model_degree'), $func), $args);
	}
	/**
	 * detail 
	 * 
	...
	function detail($param) {
		...
		if (!$selected){
			$selected  = $selected + array('degree'=>array())
				;
		}

		$c['degree'] = $this->model('Model_degree')->fetch_one_by_id($id, $selected['degree']);
		if (empty($c['degree']) ){
	...
	function search($param){
		...
		$degrees = $this->model('Model_degree')->search($param, $param['page'], $param['pagesize']);
		$degrees['results'] = array_values($degrees['results']);
		return $degrees;
	}
}
