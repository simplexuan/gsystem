 ...
 * @license 
 */
class Dao_record_log extends Gsystem_dao
{
	 ...
	 * @access protected
	 */
	protected $_insert_fields = array('uid', 'old_data', 'new_data', 'mid', 'm_table_name', 'type', 'updated_at');
	 ...
	 * @access protected
	 */
	protected $_update_fields = array('id', 'uid', 'old_data', 'new_data', 'mid', 'm_table_name', 'type', 'is_deleted', 'updated_at');
	 ...
	 * @access protected
	 */
	protected $_select_fields = array('id', 'uid', 'old_data', 'new_data', 'mid', 'm_table_name', 'type', 'is_deleted', 'updated_at', 'created_at');
	 ...
	 * @access protected
	 */
	protected $_index_fields = array('idx_type_m_table_name_mid' => array(1 => 'type', 2 => 'm_table_name', 3 => 'mid',),);
	...
	function search($param = array())
	{
		...
		$equal_search_items = array('type' => 't', 'm_table_name' => 't', 'mid' => 't');
		$like_search_items = array();
		$like2_search_items = array();

		foreach ($param as $column => $v)
		{
			if (!isset($equal_search_items[$this->_remove_operator($column)]))
				continue;
